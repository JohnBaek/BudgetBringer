 ...
 * 그리드 모델
 */
const gridModel = new BudgetProcessGridBusinessUnit(props.fullDate as string , props.year as number);
/**
 * 쿼리 정보
 */
...
const loadData = () => {
  ...
  // 서버에서 대상하는 데이터를 조회한다.
  HttpService.requestGet<ResponseData<ResponseProcessBusinessUnitSummary>>(`${requestQuery.apiUri}/BusinessUnit`).subscribe({
    async next(response) {
      // 요청에 실패한경우
