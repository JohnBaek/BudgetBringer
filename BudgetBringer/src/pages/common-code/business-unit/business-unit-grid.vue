 ...
 * props 정의
 */
const title : string = 'BusinessUnit';
/**
 * 데이터 추가 다이얼로그
 */
 ...
 * 삭제할 데이터
 */
let removeItems : Array<ResponseBusinessUnit> = [];
/**
 * 수정할 데이터
 */
let updateItem: ResponseBusinessUnit;
...
const requestAddData = () => {
  ...
  // 데이터를 입력한다.
  HttpService.requestPost<ResponseData<ResponseBusinessUnit>>(requestQuery.apiUri , modelReference.value).subscribe({
    next(response) {
 ...
 * @param items 삭제할 데이터
 */
const showRemoveDialog = (items : Array<ResponseBusinessUnit>) => {
 ...
 * @param item 수정할 데이터
 */
const showUpdateDialog = (item: ResponseBusinessUnit) => {
  communicationService.inCommunication();
  updateItem = item;

  // 서버에서 대상하는 데이터를 조회한다.
  HttpService.requestGet<ResponseData<ResponseBusinessUnit>>(`${requestQuery.apiUri}/${item.id}`).subscribe({
    async next(response) {
      // 요청에 실패한경우
      if (response.result !== EnumResponseResult.success) {
        messageService.showError(`[${response.code}] ${response.message}`);
