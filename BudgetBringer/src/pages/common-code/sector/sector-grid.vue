...
import {ResponseSector} from "../../../models/responses/budgets/response-sector";
 ...
 * 데이터 추가 원본 요청 데이터
 */
const modelReference = ref<RequestSector>(new RequestSector());
/**
 * 삭제할 데이터
 */
let removeItems : Array<ResponseSector> = [];
/**
 * 수정할 데이터
 */
let updateItem: ResponseSector;
...
const requestAddData = () => {
  ...
  // 데이터를 입력한다.
  HttpService.requestPost<ResponseData<ResponseSector>>(requestQuery.apiUri , modelReference.value).subscribe({
    next(response) {
 ...
 * @param items 삭제할 데이터
 */
const showRemoveDialog = (items : Array<ResponseSector>) => {
...
const showAddDialog = () => {
  ...
  modelReference.value = new RequestSector();
}
 ...
 * @param item 수정할 데이터
 */
const showUpdateDialog = (item: ResponseSector) => {
  communicationService.notifyInCommunication();
  updateItem = item;

  // 서버에서 대상하는 데이터를 조회한다.
  HttpService.requestGet<ResponseData<ResponseSector>>(`${requestQuery.apiUri}/${item.id}`).subscribe({
    async next(response) {
      // 요청에 실패한경우
      if (response.result !== EnumResponseResult.success) {
        messageService.showError(`[${response.code}] ${response.message}`);
