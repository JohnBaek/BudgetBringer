...
public class SectorRepository : ISectorRepository
{
    ...
    /// 로그 카테고리명
    /// </summary>
    private const string LogCategory = "[Sector]";
    ...
    public SectorRepository(
        ILogger<SectorRepository> logger
    ...
    {
        ResponseData<ResponseSector> result = new ResponseData<ResponseSector>();
        try
        {
            // 요청이 유효하지 않은경우
            if (id.IsEmpty())
    ...
    public async Task<Response> UpdateAsync(string id , RequestSector request)
    {
        ...
        try
        {
            ...
            // 로그기록을 위한 데이터 스냅샷
            DbModelSector snapshot = update.ToClone()!;
    ...
    {
        ResponseData<ResponseSector> result;
        ...
        try
        {
            ...
            if(request.IsInValid())
                return new ResponseData<ResponseSector>{ Code = "ERROR_INVALID_PARAMETER", Message = request.GetFirstErrorMessage()};
            ...
            // 추가된 데이터를 조회
            ResponseData<ResponseSector> added = await GetAsync(add.Id.ToString());
